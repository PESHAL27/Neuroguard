...
def perform_full_scan() -> List[Dict[str, Any]]:
    ...
    # Preserve registered ESP32 nodes
    for esp_ip, esp_doc in existing_registered_esp.items():
        if esp_ip not in active_devices_map:
            active_devices_map[esp_ip] = (esp_doc.get("mac", "DYNAMIC-ESP32"), 12)
